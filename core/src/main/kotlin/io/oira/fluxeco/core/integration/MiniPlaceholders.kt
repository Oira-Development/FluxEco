    ...
    fun register() {
        val expansion = expansion("fluxeco") {
            audience<Player>("balance") { player, _, _ ->
                Component.text(EconomyManager.getBalance(player.uniqueId).toString()).asInsertingTag()
            }
            audience<Player>("balance_formatted") { player, _, _ ->
                Component.text(EconomyManager.getBalance(player.uniqueId).format()).asInsertingTag()
            }
